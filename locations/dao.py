...
def get_current_dislocation() -> List:
    ...
    """
    locations = []
    arrivale_dates = [None, None, None,
                      datetime.now() - timedelta(days=3), datetime.now()]
    time.sleep(2)

    for _ in range(0, 20000):  # переменная i не используется
...
def api_call():
    ...
    # список  без даты прибытия
    invoices = list(
        map(lambda x: x['invoice'] if not x['arrivale_date'] else False, locations))
    invoices = list(set(filter(lambda y: not isinstance(y, bool), invoices)))
    # Получил список УНИКАЛЬНЫХ накладных из текущей дислокации
    ...
    for i in locations:
        if i['invoice'] is None:
            i['arrivale_date'] = predicted_data[i['invoice']]
